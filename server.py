...
def glitchme():
    while(1):
      time.sleep(120)
      try:
        res = requests.get("https://deciduous-scintillating-fur.glitch.me/")
      except:
...
def sub_redeem(i, temp,names, code, ops,objective):
        global redata
        try:
          ...
          finalme=str(e)
        redata[objective][code][names[i-1]]=finalme

def redeem(code, ops,itime,data,objective):
    global redata
    print(time.ctime())
    ...
    c = len(temp)
    redata[objective]=dict()
    redata[objective][code]=dict()
    ...
    while(len(temp)!=len(redata[objective][code].keys())):
      # print(len(temp),len(redata[objective][code]))
      pass
    # print(temp,redata)
    import datetime
    ...
    # send_msg(
    #     ops, "🚀 " + str(code) +
    #     " Received Successfully...!!!\n=================================="+str(redata[objective][code]))
    embed=discord.Embed(title="🎉 Record For Code 📣 "+str(code)+ " ✅ For "+ str(objective),color=discord.Color.green())
    # print(redata[objective][code],type(redata[objective][code]))
    for k in redata[objective][code].keys():
        embed.add_field(name="😎  "+str(k) +"  🤗", value="> "+str(redata[objective][code][k])+"\n🧿 ~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~ ⛄ ~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~~-~ 🧿", inline=False)
    ...
    embed1=discord.Embed(title="🎉 Time Taken by "+ str(objective) +" For Code 📣 "+str(code)+ " ✅",color=discord.Color.blue())
...
#             await message.channel.send('==================================')
    ...
    if message.content.startswith(('^hi', '^Hi', '^Hii', '^hii','!hi', '!Hi', '!Hii', '!hii')):
        ...
        if (len(li) == 1):
            ...
            background_thread = Thread(target=redeem,
                                       args=(
                                         ...
                                         pd.read_csv(os.getenv('mains_data_sheet')),
                                         "Mains",
                                       ))
            background_thread.start()
            background_thread = Thread(target=redeem,
                                       args=(
                                           ...
                                           message.channel,
                                         datetime.datetime.now(),
                                         pd.read_csv(os.getenv('alts_data_sheet')),
                                         "Alts",
                                       ))
            background_thread.start()
            background_thread = Thread(target=redeem,
                                       args=(
                                           ...
                                           message.channel,
                                         datetime.datetime.now(),
                                         pd.read_csv(os.getenv('others_data_sheet')),
                                         "Others",
                                       ))
            background_thread.start()
            await message.channel.send(
                "❄" + str(li[1])+" 😉 Code Received check gifts channel for details ⚡")
...
loop.create_task(client.start(str(os.getenv('TOKEN'))))
